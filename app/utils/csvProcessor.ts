...
export async function parseCSVRowWithCollectorMap(
  row: CSVRow,
  collectorMap: Map<string, string>,
  uploadPhotos: boolean = false
): Promise<NewFungi | null> {
  ...
  try {
    ...
    if (uploadPhotos) {
      try {
        ...
        console.log(
          `Uploaded ${imageUrls.length} photos for ${row["Código Fungario"]}`
        );
      } catch (error) {
        console.error(
          `Error uploading photos for ${row["Código Fungario"]}:`,
          error
        );
      }
    }
    ...
    return {
      ...
      numeroCarpoforos: row["Numero de Carpóforos"]
        ? parseInt(row["Numero de Carpóforos"])
        : undefined,
      olor: row["Olor"] && row["Olor"] !== "-" ? row["Olor"] : undefined,
      sporeprint:
        row["Sporeprint"] && row["Sporeprint"] !== "-"
          ? row["Sporeprint"]
          : undefined,
      esporadaColor:
        row["Esporada (color)"] && row["Esporada (color)"] !== "-"
          ? row["Esporada (color)"]
          : undefined,
      himenio:
        row["Himenio"] && row["Himenio"] !== "-" ? row["Himenio"] : undefined,
      anillo: row["Anillo"]?.toLowerCase() === "si",
      volva: row["Volva"]?.toLowerCase() === "si",
      textura: row["Textura"] || undefined,
      notas: row["Notas"] || undefined,
      color: row["Color"] || undefined,
      largoCuerpo: row["Largo Cuerpo (cm)"]
        ? parseFloat(row["Largo Cuerpo (cm)"])
        : undefined,
      diametroSombrero: row["Diámetro Sombrero (cm)"]
        ? parseFloat(row["Diámetro Sombrero (cm)"])
        : undefined,
...
export async function parseCSVRow(
  row: CSVRow,
  uploadPhotos: boolean = false
): Promise<NewFungi | null> {
  // Skip empty rows, header rows, or rows without fungario codes
...
export async function processCSVContent(
  ...
  console.log(
    `Starting to process CSV with ${lines.length - 3} potential records...`
  );
  ...
  console.log(
    `Parsed ${rowsData.length} valid rows, found ${collectorNames.size} unique collectors`
  );
  ...
  for (let i = 0; i < rowsData.length; i++) {
    ...
    try {
      const parsedFungi = await parseCSVRowWithCollectorMap(
        row as CSVRow,
        collectorMap,
        uploadPhotos
      );
  ...
  console.log(
    `Finished processing CSV. Total records processed: ${fungi.length}`
  );
  return fungi;
}

export async function getImagesForFungi(
